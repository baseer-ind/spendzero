...
class StoreScreen extends ConsumerStatefulWidget {
  const StoreScreen({super.key, required this.categoryId, required this.storeId});
...
class _StoreScreenState extends ConsumerState<StoreScreen> {
  ...
  Widget build(BuildContext context) {
    final store = findGroceryStoreById(widget.storeId);
    if (store == null) {
      return Scaffold(
            ...
            builder: (context, ref, _) {
              ...
              final saved = ref.watch(
                wishlistProvider.select(
                  (list) => list.any((e) => e.entityId == store.id && e.appId == appId),
                ),
              );
              return IconButton(
                ...
                onPressed: () {
                  ...
                  ref.read(wishlistProvider.notifier).toggle(store.id, appId, widget.categoryId);
                },
              );
            },
          ),
      ...
      body: ListView(
        ...
        children: [
          ...
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: store.categories.map((c) => Chip(label: Text(c))).toList(),
          ),
          const SizedBox(height: 16),
          productsAsync.when(
  ...
  void _openDetailSheet(GroceryProduct item) {
  ...
  Widget _buildCartBar(BuildContext context, List<GroceryProduct> items) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(store.brandTagline, style: const TextStyle(color: Colors.white70)),
          ...
          Row(
            children: [
              ...
              Text(
                '${store.deliveryTimeMins} min · ${store.distanceKm.toStringAsFixed(1)} km',
                style: const TextStyle(color: Colors.white),
              ),
            ],
          ),
...
class _ProductSections extends StatelessWidget {
  ...
  final void Function(GroceryProduct, int) onQuantityChanged;
  ...
  Widget build(BuildContext context) {
    ...
    final byCategory = <String, List<GroceryProduct>>{};
