...
/// Bottom sheet shown when tapping a product row — description, unit,
/// rating, tags, a quantity stepper and an "Add to cart" action that
...
class _ProductDetailSheetState extends ConsumerState<ProductDetailSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    final related = allGroceryProducts
        .where((p) =>
            p.id != item.id &&
            p.storeId == item.storeId &&
            p.tags.any((t) => item.tags.contains(t)))
        .take(4)
        .toList();
          ...
          child: Column(
            ...
            children: [
              ...
              Wrap(
                ...
                runSpacing: 8,
                children: [
                  Chip(label: Text(item.unit)),
                  Chip(label: Text('${item.rating.toStringAsFixed(1)} ★ (${item.reviewCount})')),
