...
class _BrandScreenState extends ConsumerState<BrandScreen> {
  ...
  void _setQuantity(BeautyProduct item, int quantity) {
    setState(() {
      if (quantity <= 0) {
  ...
  Widget build(BuildContext context) {
    final brand = findBeautyBrandById(widget.brandId);
    if (brand == null) {
      return Scaffold(
    ...
    final productsAsync = ref.watch(productsForBeautyBrandProvider(widget.brandId));
    final reviewsAsync = ref.watch(beautyReviewsForProvider(widget.brandId));
  ...
  void _openDetailSheet(BeautyProduct item) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      builder: (context) => BeautyProductDetailSheet(
        product: item,
  ...
  Widget _buildCartBar(BuildContext context, List<BeautyProduct> items) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(brand.brandTagline, style: const TextStyle(color: Colors.white70)),
...
class _ProductSections extends StatelessWidget {
  ...
  final ValueChanged<BeautyProduct> onAdd;
  final void Function(BeautyProduct, int) onQuantityChanged;
  ...
  Widget build(BuildContext context) {
    ...
    final byCategory = <String, List<BeautyProduct>>{};
