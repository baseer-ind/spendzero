...
class RecentlyViewedShoppingBrand {
...
/// "Fashion" and "Electronics" share one catalogue and one set of
/// screens — [category] only biases the default category-chip filter,
/// the full catalogue is always browsable either way.
String? _defaultCategoryFilter(String categoryId) {
  switch (categoryId) {
    case 'demo-fashion':
      return 'Fashion';
    case 'demo-electronics':
      return 'Electronics';
    default:
      return null;
  }
...
class ShoppingHomeScreen extends ConsumerStatefulWidget {
  const ShoppingHomeScreen({super.key, required this.category});

  final SpendCategory category;

  @override
  ConsumerState<ShoppingHomeScreen> createState() => _ShoppingHomeScreenState();
}

class _ShoppingHomeScreenState extends ConsumerState<ShoppingHomeScreen> {
  ...
  Future<void> _loadRecentlyViewed() async {
    final ids = await RecentlyViewedShoppingBrand().read();
    if (!mounted) return;
    setState(() {
      _recentlyViewed = ids.map(findShoppingBrandById).whereType<ShoppingBrand>().toList();
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () async {
          ref.invalidate(shoppingBrandsProvider);
          ref.invalidate(shoppingTrendingProvider);
          ref.invalidate(shoppingTodaysOffersProvider);
        },
        child: ListView(
            ...
            if (_searchQuery.isNotEmpty)
              ...
              brandsAsync.when(
                ...
                error: (error, _) => _ErrorState(
                  ...
                  onRetry: () => ref.invalidate(shoppingBrandsProvider),
                ),
              ),
            ],
          ],
...
class _SearchResults extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final resultsAsync = ref.watch(shoppingSearchProvider(query));
              ...
              return Padding(
                ...
                child: _BrandCard(
                  ...
                  onTap: () async {
                    ...
                    if (context.mounted) {
                      context.push('/shopping/$categoryId/brand/${result.id}');
                    }
                  },
                ),
              );
            }
            final product = result as ShoppingProduct;
            return Padding(
              padding: const EdgeInsets.only(bottom: 12),
              child: ListTile(
                ...
                onTap: () {
                  context.push('/shopping/$categoryId/brand/${product.brandId}');
                },
              ),
            );
          }).toList(),
...
class _BrandRail extends StatelessWidget {
  ...
  final String title;
  final List<ShoppingBrand> brands;
  final ValueChanged<ShoppingBrand> onTap;
