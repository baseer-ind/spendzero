...
const _recentlyViewedKey = 'movies_recently_viewed_cinemas_v1';
...
class RecentlyViewedCinemaBrand {
...
class MoviesHomeScreen extends ConsumerStatefulWidget {
  const MoviesHomeScreen({super.key, required this.category});
...
class _MoviesHomeScreenState extends ConsumerState<MoviesHomeScreen> {
  ...
  List<CinemaBrand> _recentlyViewed = [];
  _SortOption _sortOption = _SortOption.relevance;

  List<CinemaBrand> _applySort(List<CinemaBrand> list) {
    final sorted = [...list];
    switch (_sortOption) {
  ...
  Future<void> _loadRecentlyViewed() async {
    final ids = await RecentlyViewedCinemaBrand().read();
    if (!mounted) return;
    setState(() {
      _recentlyViewed = ids.map(findCinemaBrandById).whereType<CinemaBrand>().toList();
    });
  }
  ...
  void _openStay(CinemaBrand stay) async {
    await RecentlyViewedCinemaBrand().recordView(stay.id);
    ref.read(personalizationProvider.notifier).recordAppView(_appIdFor(stay.id));
    if (!mounted) return;
    context.push('/movies/${widget.category.id}/cinema/${stay.id}');
  }

  @override
  Widget build(BuildContext context) {
    final staysAsync = ref.watch(cinemasProvider);
    final trendingAsync = ref.watch(moviesTrendingProvider);
    final offersAsync = ref.watch(moviesTodaysOffersProvider);
    final wishlist = ref.watch(wishlistProvider);
    final savedStays = wishlist
        .where((e) => e.categoryId == widget.category.id)
        .map((e) => findCinemaBrandById(e.entityId))
        .whereType<CinemaBrand>()
        .toList();
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () async {
          ...
          ref.invalidate(moviesTrendingProvider);
          ref.invalidate(moviesTodaysOffersProvider);
        },
        child: ListView(
          ...
          children: [
            ...
            if (_searchQuery.isNotEmpty)
              ...
              if (actionPacked().isNotEmpty) ...[
                _MovieCollectionRail(
                  title: 'Action Packed',
                  subtitle: 'Edge-of-the-seat thrills',
                  items: actionPacked(),
                  categoryId: widget.category.id,
                  accentColor: (context) => Colors.orangeAccent,
                ),
                const SizedBox(height: 20),
              ],
              if (familyWatch().isNotEmpty) ...[
                _MovieCollectionRail(
                  title: 'Family Watch',
              ...
              if (premiumExperience().isNotEmpty) ...[
                _MovieCollectionRail(
                  title: 'Premium Experience',
                  subtitle: 'IMAX & 4DX for the full effect',
                  items: premiumExperience(),
                  categoryId: widget.category.id,
...
class _SearchResults extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final resultsAsync = ref.watch(moviesSearchProvider(query));
            ...
            if (result is CinemaBrand) {
              return Padding(
                ...
                child: _StayCard(
                  ...
                  onTap: () async {
                    await RecentlyViewedCinemaBrand().recordView(result.id);
                    if (context.mounted) {
                      context.push('/movies/$categoryId/cinema/${result.id}');
                    }
                  },
                ),
              );
            }
            final room = result as Movie;
            return Padding(
              padding: const EdgeInsets.only(bottom: 12),
              child: ListTile(
                ...
                subtitle: Text(formatPaise(room.pricePaise)),
                onTap: () {
                  context.push('/movies/$categoryId/cinema/${room.cinemaId}');
                },
              ),
            );
          }).toList(),
...
class _StayRail extends StatelessWidget {
  ...
  final String title;
  final List<CinemaBrand> stays;
  final ValueChanged<CinemaBrand> onTap;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(item.title, maxLines: 2, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.bodyMedium),
          const Spacer(),
          Row(
            children: [
...
class _MovieCollectionRail extends ConsumerWidget {
  const _MovieCollectionRail({
    required this.title,
              ...
              return Material(
                ...
                child: InkWell(
                  ...
                  onTap: () async {
                    ...
                    if (context.mounted) {
                      context.push('/movies/$categoryId/cinema/${item.cinemaId}');
                    }
                  },
                  child: Container(
                    width: 170,
...
class _StayCard extends ConsumerWidget {
  const _StayCard({required this.stay, required this.onTap, this.categoryId = 'movies'});
